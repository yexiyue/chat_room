  ...
  <div class="window">
    ...
    <div class="container">
      <Show-vue v-if="route.query.id=='all'">
        <template #default>
          <span v-for="item in msgListMap.get('allToAll')" :key="item.dateTime" :class="[item.whoSendId==myId?'right':'']">
      ...
      <Show-vue v-else>
        <template #default>
          <span v-for="item in msgListMap.get(String(myId+route.query.id))" :key="item.dateTime" :class="[item.whoSendId==myId?'right':'']">
            <div>{{item.whoSendId}}</div>
            {{item.msg}}
          </span>
        </template>
...
const msgListMap:Map<string,Set<MsgData>>=reactive(new Map())

store.$subscribe(()=>{
  store.msgList.forEach(v=>{
    const list:Set<MsgData>=new Set()
    if(v.whoSendId=='all' || v.targetId=='all'){
      if(msgListMap.get('allToAll')){
        const list=msgListMap.get('allToAll')
        list?.add(v)
      }else{
        list.add(v)
        msgListMap.set('allToAll',list)
      }
    }

    if((v.whoSendId==myId && v.targetId==route.query.id) || (v.whoSendId==route.query.id && v.targetId==myId) ){
      if(msgListMap.get(String(myId+route.query.id))){
        const list=msgListMap.get(String(myId+route.query.id))
        list?.add(v)
        console.log(msgListMap.get(String(myId+route.query.id)))
      }else{
        list.add(v)
        msgListMap.set(String(myId+route.query.id),list)
      }
    }
  })
